...
type
  ...
  Client* = ref object
    ...
    preFrame*: proc(client: Client)
    postFrame*: proc(client: Client)

    platform*: ClientPlatformData

func newClientBase*(width, height: float): Client =
  Client(
    width: width,
    height: height,
    widthPrevious: width,
...
template disableCursor*(client: Client): untyped =
  client.hideCursor()
  client.pinCursorToCenter()
  client.confineCursor()

template enableCursor*(client: Client): untyped =
  client.unconfineCursor()
  client.unpinCursorFromCenter()
  client.showCursor()
...
template startEventLoop*(client: Client, code: untyped): untyped =
  ...
  while not client.shouldClose:
    ...
    if client.preFrame != nil:
      client.preFrame(client)
    code
    if client.postFrame != nil:
      client.postFrame(client)
