...
func transform2*(xx, xy, yx, yy, ox, oy: float32): Transform2 =
...
func transform2*(x, y, origin: Vec2): Transform2 =
  result.elements[0] = x
  result.elements[1] = y
  result.elements[2] = origin

func transform2*(rot: float32, pos: Vec2): Transform2 =
  let cr = cos(rot)
  let sr = sin(rot)
...
func transform2*(rot: float32, scale: Vec2, skew: float32, pos: Vec2): Transform2 =
...
func transform2*(): Transform2 =
  result.elements[0][0] = 1.0
  result.elements[1][1] = 1.0

template `[]`*(self: Transform2, i: int): untyped = self.elements[i]
template `[]=`*(self: Transform2, i: int, v: float32): untyped = self.elements[i] = v

func tdotx*(self: Transform2, v: Vec2): float32 =
  self.elements[0][0] * v.x + self.elements[1][0] * v.y

func tdoty*(self: Transform2, v: Vec2): float32 =
  self.elements[0][1] * v.x + self.elements[1][1] * v.y

func basisXform*(self: Transform2, v: Vec2): Vec2 =
  vec2(self.tdotx(v), self.tdoty(v))

func basisXformInv*(self: Transform2, v: Vec2): Vec2 =
  vec2(self.elements[0].dot(v), self.elements[1].dot(v))

func xform*(self: Transform2, v: Vec2): Vec2 =
  vec2(self.tdotx(v), self.tdoty(v)) + self.elements[2]

func xformInv*(self: Transform2, v: Vec2): Vec2 =
  let ve = v - self.elements[2]
  vec2(self.elements[0].dot(ve), self.elements[1].dot(ve))

func xform*(self: Transform2, r: Rect2): Rect2 =
  let x = self.elements[0] * r.size.x
  let y = self.elements[1] * r.size.y
  let pos = self.xform(r.position)

  result.position = pos
  result.expandTo(pos + x)
  result.expandTo(pos + y)
  result.expandTo(pos + x + y)

func `==`*(self, other: Transform2): bool =
  for i in 0 ..< 3:
    if self.elements[i] != other.elements[i]:
      return false
  true

func `*=`*(self: var Transform2, other: Transform2) =
...
func `*`*(self, other: Transform2): Transform2 =
  result = self
  result *= other

func `*=`*(self: var Transform2, v: float32) =
  self.elements[0] *= v
  self.elements[1] *= v
  self.elements[2] *= v

func `*`*(self: Transform2, v: float32): Transform2 =
  result = self
  result *= v

func invert*(self: var Transform2) =
  swap(self.elements[0][1], self.elements[1][0])
  self.elements[2] = self.basisXform(-self.elements[2])

func inverse*(self: Transform2): Transform2 =
  result = self
  result.invert()

func basisDeterminant*(self: Transform2): float32 =
  self.elements[0].x * self.elements[1].y - self.elements[0].y * self.elements[1].x

func getOrigin*(self: Transform2): Vec2 =
  self.elements[2]

func setOrigin*(self: var Transform2, origin: Vec2) =
  self.elements[2] = origin

func affineInvert*(self: var Transform2) =
...
func getRotation*(self: Transform2): float32 =
  arctan2(self.elements[0].y, self.elements[0].x)

func rotate*(self: var Transform2, phi: float32) =
  self = transform2(phi, vec2()) * self

func rotated*(self: Transform2, phi: float32): Transform2 =
  result = self
  result.rotate(phi)

func getSkew*(self: Transform2): float32 =
  let det = self.basisDeterminant()
  arccos(self.elements[0].normalized().dot(sgn(det).float32 * self.elements[1].normalized())) - Pi * 0.5f

func setSkew*(self: var Transform2, angle: float32) =
  let det = self.basisDeterminant()
  self.elements[1] = sgn(det).float32 * self.elements[0].rotated((Pi * 0.5f + angle)).normalized() * self.elements[1].length()

func setRotationAndScale*(self: var Transform2, rot: float32, scale: Vec2) =
...
func setRotationScaleAndSkew*(self: var Transform2, rot: float32, scale: Vec2, skew: float32) =
  self.elements[0][0] = cos(rot) * scale.x
  self.elements[1][1] = cos(rot + skew) * scale.y
  self.elements[1][0] = -sin(rot + skew) * scale.y
  self.elements[0][1] = sin(rot) * scale.x

func translate*(self: var Transform2, translation: Vec2) =
  self.elements[2] += self.basisXform(translation)

func translate*(self: var Transform2, x, y: float32) =
  self.translate(vec2(x, y))

func translated*(self: Transform2, translation: Vec2): Transform2 =
  result = self
  result.translate(translation)

func translated*(self: Transform2, x, y: float32): Transform2 =
  result = self
  result.translate(x, y)

func orthonormalize*(self: var Transform2) =
...
func orthonormalized*(self: Transform2): Transform2 =
  result = self
...
func lookingAt*(self: Transform2, target: Vec2): Transform2 =
  result = transform2(self.getRotation(), self.getOrigin())
...
func interpolatedWith*(self, other: Transform2, c: float32): Transform2 =
  # extract parameters
  ...
  # construct matrix
  result = transform2(v.angle(), p1.lerped(p2, c))
