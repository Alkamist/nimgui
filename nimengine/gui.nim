...
proc getClipboardText(user_data: pointer): cstring {.cdecl.} =
  var osWindow = cast[Window](user_data)
  osWindow.clipboard.cstring

proc setClipboardText(user_data: pointer, text: cstring) {.cdecl.} =
  var osWindow = cast[Window](user_data)
  osWindow.clipboard = $text
...
proc newGui*(osWindow: Window): Gui =
  result = Gui(
    osWindow: osWindow,
    imguiContextPtr: cast[pointer](ImGui_CreateContext()),
  )
  ImGui_SetCurrentContext(result.imguiContext)
  var io = ImGui_GetIO()
  setDefaultStyle()
  io.GetClipboardTextFn = getClipboardText
  io.SetClipboardTextFn = setClipboardText
  io.ClipboardUserData = cast[pointer](osWindow)
...
proc beginFrame*(gui: Gui) =
  ImGui_SetCurrentContext(gui.imguiContext)
  ImGui_ImplOpenGL3_NewFrame()
  var io = ImGui_GetIO()

  let osWindow = gui.osWindow

  io.DisplaySize = osWindow.size.toImVec2

  if osWindow.size.x > 0 and osWindow.size.y > 0:
    let pixelDensity = osWindow.pixelDensity
    io.DisplayFramebufferScale = imVec2(pixelDensity, pixelDensity)

  if osWindow.deltaTime > 0:
    io.DeltaTime = osWindow.deltaTime
  else:
    io.DeltaTime = 1.0 / 60.0

  if osWindow.gainedFocus:
    io.AddFocusEvent(true)
  elif osWindow.lostFocus:
    io.AddFocusEvent(false)

  template updateKeyModifiers() =
    io.AddKeyEvent(ImGuiKey_ModCtrl, osWindow.keyDown(LeftControl) or osWindow.keyDown(RightControl))
    io.AddKeyEvent(ImGuiKey_ModShift, osWindow.keyDown(LeftShift) or osWindow.keyDown(RightShift))
    io.AddKeyEvent(ImGuiKey_ModAlt, osWindow.keyDown(LeftAlt) or osWindow.keyDown(RightAlt))
    io.AddKeyEvent(ImGuiKey_ModSuper, osWindow.keyDown(LeftMeta) or osWindow.keyDown(RightMeta))

  for key in osWindow.keyPresses:
    updateKeyModifiers()
    io.AddKeyEvent(key.toImGuiKey, true)

  for key in osWindow.keyReleases:
    updateKeyModifiers()
    io.AddKeyEvent(key.toImGuiKey, false)

  if osWindow.text != "":
    for rune in osWindow.text.runes:
      io.AddInputCharactersUTF8(rune.toUTF8.cstring)

  if osWindow.mouseMoved:
    io.AddMousePosEvent(osWindow.mousePosition.x, osWindow.mousePosition.y)

  if osWindow.mouseEntered:
    io.AddMousePosEvent(gui.mouseExitPosition.x, gui.mouseExitPosition.y)

  if osWindow.mouseExited:
    gui.mouseExitPosition = osWindow.mousePosition
    io.AddMousePosEvent(-FLT_MAX, -FLT_MAX)

  for button in osWindow.mousePresses:
    updateKeyModifiers()
  ...
  for button in osWindow.mouseReleases:
    updateKeyModifiers()
    let imguiButton = button.toImGuiMouseButton
    if imguiButton >= 0 and imguiButton < ImGuiMouseButton_COUNT:
      io.AddMouseButtonEvent(imguiButton, false)

  if osWindow.mouseWheelMoved:
    io.AddMouseWheelEvent(osWindow.mouseWheel.x, osWindow.mouseWheel.y)
...
proc endFrame*(gui: Gui) =
  let sizePixels = gui.osWindow.size * gui.osWindow.pixelDensity
