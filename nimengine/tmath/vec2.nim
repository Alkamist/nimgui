...
template x*[V: Vec2Tuple](tv: V): untyped = tv[0]
template y*[V: Vec2Tuple](tv: V): untyped = tv[1]

template asFloat*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asFloat, y: v.y.asFloat)

template asFloat64*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asFloat64, y: v.y.asFloat64)

template asFloat32*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asFloat32, y: v.y.asFloat32)

template asInt*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asInt, y: v.y.asInt)

template asInt64*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asInt64, y: v.y.asInt64)

template asInt32*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asInt32, y: v.y.asInt32)

template asInt16*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asInt16, y: v.y.asInt16)

template asInt8*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asInt8, y: v.y.asInt8)

template asUInt*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asUInt, y: v.y.asUInt)

template asUInt64*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asUInt64, y: v.y.asUInt64)

template asUInt32*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asUInt32, y: v.y.asUInt32)

template asUInt16*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asUInt16, y: v.y.asUInt16)

template asUInt8*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.asUInt8, y: v.y.asUInt8)

template `+`*[V: SomeVec2](tv: V): untyped =
  tv
...
template vec2BinaryOperator(op: untyped): untyped =
  ...
  template op*[A: SomeVec2, B: SomeNumber](ta: A, tb: B): untyped =
    let a = ta
    let b = tb
...
template vec2BinaryEqualsOperator(opEq, op: untyped): untyped =
  ...
  template opEq*[A: SomeVec2, B: SomeNumber](ta: var A, tb: B) = ta = op(ta, tb)
...
template `+`*[A: SomeNumber, B: SomeVec2](ta: A, tb: B): untyped =
  let a = ta
  let b = tb
  (x: a + b.x, y: a + b.y)
...
template length*[V: SomeVec2](tv: V): untyped =
  let v = tv.asFloat
  (v.x * v.x + v.y * v.y).sqrt

template lengthSquared*[V: SomeVec2](tv: V): untyped =
  let v = tv
  v.x * v.x + v.y * v.y
...
template rotate*[V: SomeVec2, P: SomeNumber](tv: V, tphi: P): untyped =
  let v = tv.asFloat
...
template round*[V: SomeVec2](tv: V): untyped =
  let v = tv
  (x: v.x.round, y: v.y.round)

template angle*[V: SomeVec2](tv: V): untyped =
  let v = tv.asFloat
  arctan2(v.y, v.x)

template isNormalized*[V: SomeVec2](tv: V): bool =
  v.lengthSquared ~= 1.0

template normalize*[V: SomeVec2](tv: V): untyped =
  var res = tv.asFloat
...
template lerp*[A, B: SomeVec2, W: SomeNumber](ta: A, tb: B, tweight: W): untyped =
  let a = ta.asFloat
  let b = tb.asFloat
...
template limit*[V: SomeVec2, L: SomeNumber](tv: V, tlimit: L): untyped =
  var res = tv.asFloat
